...
def exec_event_make_choice(self, node:TaskNode, func):
    ...
    special_phase = ["选择后", "等级提升", "成功时", "判定", "根据"] if self._get_using_cfg("other_task")["language"] == "ch" else ["Select to gain", "Pass to level up", "Pass to gain", "check to gain", "depending on"] # TODO 拉了坨大的 属于临时解决方案 后面应该改图像匹配或者加一个配置文件
    for res in results:
